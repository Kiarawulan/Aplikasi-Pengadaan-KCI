  ...
  useEffect(() => {
    api.get("/templates")
      .then((response) => setDocs(response.data.map((item: any) => {
        ...
        id: item.id,
        nama: item.nama,
        kategori: subkategori ? `${kategoriUtama} â€¢ ${subkategori}` : kategoriUtama,
        tipe: item.tipe,
        ukuran: item.ukuran,
    ...
    <div>
      ...
      <div className="bg-white rounded-2xl border border-gray-100 overflow-hidden shadow-sm">
        <div className="bg-[#252271] px-5 py-4 flex items-center justify-between">
          ...
          <div className="relative"><Search size={12} className="absolute left-3 top-1/2 -translate-y-1/2 text-white/50" /><input value={search} onChange={(event) => setSearch(event.target.value)} className="bg-white/10 border border-white/20 pl-8 pr-3 py-1.5 rounded-xl text-[11px] focus:outline-none w-40 text-white placeholder-white/50" placeholder="Pencarian" /></div>
        </div>
          ...
          <tbody className="divide-y divide-gray-50">
            {docs.filter((document) => `${document.nama} ${document.kategori} ${document.deskripsi || ""}`.toLowerCase().includes(search.toLowerCase())).map((d, i) => (
              <tr key={d.id} className={i % 2 === 1 ? "bg-gray-50/40" : ""}>
                ...
                <td className="px-4 py-3.5">
                  <span className="flex items-center gap-1 text-[10.5px] text-gray-400"><Download size={11} /> Metadata template</span>
                </td>
              </tr>
            ))}
          </tbody>
