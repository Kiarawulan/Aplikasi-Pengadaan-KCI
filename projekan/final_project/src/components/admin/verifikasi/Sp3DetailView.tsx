...
export const Sp3DetailView: React.FC<Sp3DetailViewProps> = ({
  ...
  const infoData = {
    nppNo: item?.nppNo || item?.sp3 || item?.id || '-',
    title: item?.title || item?.judul || item?.nama || '-',
    rkapValue: item?.rkap || item?.nilaiRkap || item?.nilai || item?.nominal || '-',
    vendorName: item?.vendor || item?.vendorName || '-',
    department: item?.dept || item?.departemen || '-',
    division: item?.division || item?.divisi || '-',
    status: item?.status || '-',
    taxType: item?.taxType || item?.typeTax || '-',
    taxValue: item?.tax || item?.nilaiTax || item?.taxValue || '-',
    tipePemilihan: item?.tipePemilihan || item?.metode || '-',
    rabNo: item?.rabNo || '-',
    rabDate: item?.rabDate || '-',
    kakNo: item?.kakNo || item?.noKak || '-',
    kakDate: item?.kakDate || item?.tanggalKak || '-',

    prNo: item?.prNo || '-',
    prDate: item?.prDate || '-',
    justifikasiBrgNo: item?.justifikasiBrgNo || '-',
    justifikasiBrgDate: item?.justifikasiBrgDate || '-',
    miNo: item?.miNo || item?.nomorMemoInternal || '-',
    miDate: item?.miDate || item?.tanggalMemo || '-',
    miPerihal: item?.miPerihal || '-',

    // Files
    filePr: item?.filePr || '',
    fileRab: item?.fileRab || '',
    fileJustifikasiBrg: item?.fileJustifikasiBrg || '',
    fileKak: item?.fileKak || '',
    fileMi: item?.fileMi || '',
  };
