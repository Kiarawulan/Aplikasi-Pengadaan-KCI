...
import { useUserVerificationNotifications } from "@/hooks/useUserVerificationNotifications";
...
export function Sidebar({ screen, backScreen, onNavigate, collapsed, onToggleCollapse }: {
  ...
  const { counts, markRead } = useUserVerificationNotifications();
  return (
    <div
    ...
    >
      ...
      <nav className="flex-1 flex flex-col gap-0.5 px-2 py-1 overflow-y-auto overflow-x-hidden">
        ...
        {hasPermission("pengadaan", "viewer") && <NavItem icon={<FileText size={15} />} label="RUP" active={screen === "rup-list"} collapsed={collapsed} onClick={() => { markRead("pengadaan"); onNavigate("rup-list"); }} badge={counts.pengadaan} />}
        {hasPermission("pengadaan", "viewer") && <DaftarPengadaanNavItem screen={screen} backScreen={backScreen} onNavigate={onNavigate} collapsed={collapsed} badge={counts.pengajuanDana} onRead={() => markRead("pengajuanDana")} />}
        {hasPermission("pengujian", "viewer") && <NavItem icon={<img src={pengujianIcon} alt="" aria-hidden="true" className="size-[15px] object-contain brightness-0 invert" />} label="Daftar Pengujian" active={screen === "daftar-pengujian" || (["pd-detail", "pr-detail"].includes(screen) && backScreen === "daftar-pengujian")} collapsed={collapsed} onClick={() => { markRead("pengujian"); onNavigate("daftar-pengujian"); }} badge={counts.pengujian} />}
        {hasPermission("pembayaran", "viewer") && <DaftarPembayaranNavItem screen={screen} backScreen={backScreen} onNavigate={onNavigate} collapsed={collapsed} badge={counts.pembayaran} onRead={() => markRead("pembayaran")} />}
