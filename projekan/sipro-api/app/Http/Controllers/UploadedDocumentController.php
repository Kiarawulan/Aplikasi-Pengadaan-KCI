...
{
    public function index(Request $request, Pengadaan $pengadaan)
    {
        $this->authorizeAccess($request, $pengadaan);
        return response()->json(['success' => true, 'data' => $pengadaan->documents()->latest()->get()]);
    }
    ...
    public function store(Request $request, Pengadaan $pengadaan)
    {
        ...
        $path = $file->store("pengadaan/{$pengadaan->id}", 'public');
        $document = UploadedDocument::create(['pengadaan_id' => $pengadaan->id, 'stage' => $request->stage, 'original_name' => $file->getClientOriginalName(), 'path' => $path, 'mime_type' => $file->getMimeType(), 'size' => $file->getSize(), 'uploaded_by' => $request->user()->id]);
